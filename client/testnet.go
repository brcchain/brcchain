...
import (
	...
	"github.com/brcchain/brcchain/testutil/network"
)
...
func initTestnetFiles(
	...
	for i := 0; i < args.numValidators; i++ {
		...
		coins := sdk.Coins{
			sdk.NewCoin(ethermint.AttoBrc, accStakingTokens),
		}
		...
		createValMsg, err := stakingtypes.NewMsgCreateValidator(
			sdk.ValAddress(addr),
			valPubKeys[i],
			sdk.NewCoin(ethermint.AttoBrc, valTokens),
		...
		customAppTemplate, customAppConfig := config.AppConfig(ethermint.AttoBrc)
	...
	if err := initGenFiles(clientCtx, mbm, args.chainID, ethermint.AttoBrc, genAccounts, genBalances, genFiles, args.numValidators); err != nil {
		return err
	}
